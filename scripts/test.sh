...
for l in ${libs[@]}; do
	docker run -v ~/.go:/go -v ${DIR}/..:/go/src/github.com/commnerd/SpiderWeb -v ${DIR}/..:/workspace -w /workspace/src/lib/$l -it --rm --name sw-SpiderWeb-lib-tester golang bash -c "go get -d && go test"
done
